...
import { postVideoToFacebook } from "@/app/actions/platforms/postVideoToFacebook";
...
export function PostToFacebookVideoButton({
  carouselId,
  videoBlob,
}: {
  carouselId: string;
  ...
  async function handlePost() {
    ...
    setLoading(true);
    try {
      const pathRes = await getVideoUploadPath(carouselId, "facebook");
      if (!pathRes.ok) {
      ...
      const result = await postVideoToFacebook(carouselId, pathRes.path);
      if (result.ok) {
        setSuccess(true);
  ...
  if (success && postUrl) {
    return (
      ...
      >
        <PlatformIcon platform="facebook" className="size-3.5" />
        <span>View post</span>
      </a>
    );
  }
