...
import { postVideoToInstagram } from "@/app/actions/platforms/postVideoToInstagram";
...
export function PostToInstagramVideoButton({
  carouselId,
  videoBlob,
}: {
  carouselId: string;
  ...
  async function handlePost() {
    ...
    setLoading(true);
    try {
      const pathRes = await getVideoUploadPath(carouselId, "instagram");
      if (!pathRes.ok) {
      ...
      const result = await postVideoToInstagram(carouselId, pathRes.path);
      if (result.ok) {
        setSuccess(true);
        setPostUrl(result.permalink ?? "https://www.instagram.com/");
      } else {
  ...
  return (
    <span className="inline-flex items-center gap-2">
      <Button
        ...
        title="Post video to Instagram"
      >
        {loading ? <Loader2Icon className="size-3.5 animate-spin" /> : <PlatformIcon platform="instagram" className="size-3.5" />}
      </Button>
