...
import { extractChromeChipStyle, mergeChromeChipIntoZone } from "@/lib/renderer/chromeChipStyle";
...
export function SlideEditForm({
  ...
  const normalizeExtraTextZone = useCallback((raw: unknown): ExtraTextZone | null => {
    ...
    return {
      ...
      fontWeight: clampFontWeight(Math.round(n(z.fontWeight, 700))),
  ...
  const [suppressedExtraTextZoneIds, setSuppressedExtraTextZoneIds] = useState<string[]>(() =>
    parseExtraTextZonesSuppressedIds((slide.meta as Record<string, unknown> | null)?.extra_text_zones_suppressed_ids)
  );
  ...
  type CounterZoneOverride = {
    top?: number;
    right?: number;
    fontSize?: number;
  } & ReturnType<typeof extractChromeChipStyle>;
  type WatermarkZoneOverride = {
  ...
  const [counterZoneOverride, setCounterZoneOverride] = useState<CounterZoneOverride | undefined>(() => {
    const m = slide.meta as { counter_zone_override?: Record<string, unknown> } | null;
    const raw = m?.counter_zone_override;
    if (!raw || typeof raw !== "object" || Object.keys(raw).length === 0) return undefined;
    return {
      ...(raw.top != null && { top: Number(raw.top) }),
      ...(raw.right != null && { right: Number(raw.right) }),
      ...(raw.fontSize != null && { fontSize: Number(raw.fontSize) }),
      ...extractChromeChipStyle(raw),
    } as CounterZoneOverride;
  });
  const [watermarkZoneOverride, setWatermarkZoneOverride] = useState<WatermarkZoneOverride | undefined>(() => {
    const m = slide.meta as { watermark_zone_override?: Record<string, unknown> } | null;
    const raw = m?.watermark_zone_override;
    if (!raw || typeof raw !== "object" || Object.keys(raw).length === 0) return undefined;
    const pos = raw.position;
    const validPos = ["top_left", "top_right", "bottom_left", "bottom_right", "custom"].includes(String(pos));
    return {
      ...(validPos && { position: pos as WatermarkZoneOverride["position"] }),
      ...(raw.logoX != null && { logoX: Number(raw.logoX) }),
      ...(raw.logoY != null && { logoY: Number(raw.logoY) }),
      ...(raw.fontSize != null && { fontSize: Number(raw.fontSize) }),
      ...(raw.maxWidth != null && { maxWidth: Number(raw.maxWidth) }),
      ...(raw.maxHeight != null && { maxHeight: Number(raw.maxHeight) }),
      ...(typeof raw.color === "string" && /^#([0-9A-Fa-f]{3}){1,2}$/.test(raw.color) && { color: raw.color }),
      ...extractChromeChipStyle(raw),
    } as WatermarkZoneOverride;
  });
  const [madeWithZoneOverride, setMadeWithZoneOverride] = useState<MadeWithZoneOverride | undefined>(() => {
    const m = slide.meta as { made_with_zone_override?: Record<string, unknown> } | null;
    const raw = m?.made_with_zone_override;
    if (!raw || typeof raw !== "object" || Object.keys(raw).length === 0) return undefined;
    const x = raw.x != null ? Math.min(Number(raw.x), 968) : undefined;
    const y = raw.y != null ? Math.min(Number(raw.y), 1900) : undefined;
  ...
  const [extraTextHighlightsByZone, setExtraTextHighlightsByZone] = useState<Record<string, HighlightSpan[]>>(() =>
    parseExtraTextHighlightsMap((slide.meta as Record<string, unknown> | null)?.extra_text_highlights)
  );
  const [extraTextHighlightStylesByZone, setExtraTextHighlightStylesByZone] = useState<Record<string, "text" | "background">>(() =>
    parseExtraTextHighlightStylesMap((slide.meta as Record<string, unknown> | null)?.extra_text_highlight_styles)
  );
  const [extraTextOutlineStrokesByZone, setExtraTextOutlineStrokesByZone] = useState<Record<string, number>>(() =>
    parseExtraTextOutlineStrokesMap((slide.meta as Record<string, unknown> | null)?.extra_text_outline_strokes)
  );
  const [extraTextBoldWeightsByZone, setExtraTextBoldWeightsByZone] = useState<Record<string, number>>(() =>
    parseExtraTextBoldWeightsMap((slide.meta as Record<string, unknown> | null)?.extra_text_bold_weights)
  );
  ...
  const [extraLayoutPopoverOpenZoneId, setExtraLayoutPopoverOpenZoneId] = useState<string | null>(null);
  ...
  const savedHighlightSelectionRef = useRef<{ field: "headline" | "body" | string; start: number; end: number } | null>(null);
  ...
  const buildEditorDirtySnapshotString = useCallback(
    (templateIdForSnapshot?: string | null) => {
      ...
      return JSON.stringify({
        ...
        extraTextZonesSuppressed: [...suppressedExtraTextZoneIds].sort(),
        extraTextZonesPersist: JSON.stringify(customExtraTextZones),
        extraTextValuesSnapshot: JSON.stringify(extraTextValues),
      });
    },
    [
      headline,
      ...
      slideOverlayShapes,
      overlayShapesReplaceTemplate,
      suppressedExtraTextZoneIds,
      customExtraTextZones,
      extraTextValues,
    ]
  );
  ...
  const suppressedExtraTextZoneSet = useMemo(
    () => new Set(suppressedExtraTextZoneIds),
    [suppressedExtraTextZoneIds]
  );
  const extraTextZones = useMemo(() => {
    const suppressed = suppressedExtraTextZoneSet;
    const out: ExtraTextZone[] = [];
    for (const t of templateExtraTextZones) {
      const o = customExtraTextZones.find((c) => c.id === t.id);
      const merged = (o ? { ...t, ...o } : { ...t }) as ExtraTextZone;
      if (!suppressed.has(merged.id)) out.push(merged);
    }
    for (const c of customExtraTextZones) {
      if (!templateExtraTextZones.some((t) => t.id === c.id) && !suppressed.has(c.id)) {
        out.push(c);
      }
    }
    return out;
  }, [templateExtraTextZones, customExtraTextZones, suppressedExtraTextZoneSet]);

  const allExtraTextZoneRows = useMemo(() => {
    const rows: { zone: ExtraTextZone; fromTemplate: boolean }[] = [];
    for (const t of templateExtraTextZones) {
      const o = customExtraTextZones.find((c) => c.id === t.id);
      rows.push({ zone: (o ? { ...t, ...o } : { ...t }) as ExtraTextZone, fromTemplate: true });
    }
    for (const c of customExtraTextZones) {
      if (!templateExtraTextZones.some((t) => t.id === c.id)) {
        rows.push({ zone: c, fromTemplate: false });
      }
    }
    return rows;
  }, [templateExtraTextZones, customExtraTextZones]);

  const allKnownExtraZoneIds = useMemo(() => {
    const s = new Set<string>();
    for (const z of extraTextZones) s.add(z.id);
    for (const id of suppressedExtraTextZoneIds) s.add(id);
    for (const z of customExtraTextZones) s.add(z.id);
    return s;
  }, [extraTextZones, suppressedExtraTextZoneIds, customExtraTextZones]);

  const editorLiveExtraTextMeta = useMemo(() => {
    const base: Record<string, unknown> = {
      ...(slide.meta as Record<string, unknown> | null),
      extra_text_values: extraTextValues,
    };
    if (customExtraTextZones.length > 0) {
      base.extra_text_zones = customExtraTextZones;
    } else {
      delete base.extra_text_zones;
    }
    if (suppressedExtraTextZoneIds.length > 0) {
      base.extra_text_zones_suppressed_ids = [...suppressedExtraTextZoneIds];
    } else {
      delete base.extra_text_zones_suppressed_ids;
    }
    const hl: Record<string, HighlightSpan[]> = {};
    for (const id of allKnownExtraZoneIds) {
      const spans = extraTextHighlightsByZone[id];
      if (!spans?.length) continue;
      const plain = extraTextValues[id] ?? "";
      const norm = normalizeHighlightSpansToWords(plain, spans);
      if (norm.length) hl[id] = norm;
    }
    if (Object.keys(hl).length > 0) base.extra_text_highlights = hl;
    else delete base.extra_text_highlights;

    const st: Record<string, "text" | "background"> = {};
    for (const id of allKnownExtraZoneIds) {
      if (extraTextHighlightStylesByZone[id] === "background") st[id] = "background";
    }
    if (Object.keys(st).length > 0) base.extra_text_highlight_styles = st;
    else delete base.extra_text_highlight_styles;

    const outl: Record<string, number> = {};
    for (const id of allKnownExtraZoneIds) {
      const v = extraTextOutlineStrokesByZone[id];
      if (typeof v === "number" && v > 0 && v <= 8) outl[id] = v;
    }
    if (Object.keys(outl).length > 0) base.extra_text_outline_strokes = outl;
    else delete base.extra_text_outline_strokes;

    const bw: Record<string, number> = {};
    for (const id of allKnownExtraZoneIds) {
      const w = extraTextBoldWeightsByZone[id];
      if (typeof w === "number" && w !== 700 && w >= FONT_WEIGHT_MIN && w <= FONT_WEIGHT_MAX) bw[id] = w;
    }
    if (Object.keys(bw).length > 0) base.extra_text_bold_weights = bw;
    else delete base.extra_text_bold_weights;
    ...
    customExtraTextZones,
    extraTextValues,
    extraTextHighlightsByZone,
    extraTextHighlightStylesByZone,
    extraTextOutlineStrokesByZone,
    extraTextBoldWeightsByZone,
    suppressedExtraTextZoneIds,
    allKnownExtraZoneIds,
  ]);

  const templateConfig = useMemo(() => {
    if (!templateConfigBase) return null;
    const suppressed = new Set(suppressedExtraTextZoneIds);
    const baseZones = templateConfigBase.textZones.filter(
      (z) => z.id === "headline" || z.id === "body" || !suppressed.has(z.id)
    );
    const baseZonesChanged = baseZones.length !== templateConfigBase.textZones.length;
    if (extraTextZones.length === 0 && !baseZonesChanged) return templateConfigBase;
    if (extraTextZones.length === 0) {
      return { ...templateConfigBase, textZones: baseZones };
    }
    const merged = [...baseZones];
  ...
  }, [templateConfigBase, extraTextZones, suppressedExtraTextZoneIds]);
  ...
  useEffect(() => {
    ...
    const metaRec = slide.meta as Record<string, unknown> | null;
    const suppressedIdsArr = parseExtraTextZonesSuppressedIds(metaRec?.extra_text_zones_suppressed_ids);
    setSuppressedExtraTextZoneIds(suppressedIdsArr);
    ...
    setExtraTextHighlightsByZone(parseExtraTextHighlightsMap(metaRec?.extra_text_highlights));
    setExtraTextHighlightStylesByZone(parseExtraTextHighlightStylesMap(metaRec?.extra_text_highlight_styles));
    setExtraTextOutlineStrokesByZone(parseExtraTextOutlineStrokesMap(metaRec?.extra_text_outline_strokes));
    setExtraTextBoldWeightsByZone(parseExtraTextBoldWeightsMap(metaRec?.extra_text_bold_weights));
  ...
  const handleAddExtraTextArea = () => {
    if (allExtraTextZoneRows.length >= 12) return;
    ...
    const w = Math.max(200, Math.min(980, Math.round(baseZone?.w ?? 920)));
    const h = Math.max(60, Math.min(420, Math.round((baseZone?.h ?? 220) * 0.6)));
    const rot = baseZone?.rotation ?? 0;
    const rawX = Math.round((1080 - w) / 2);
    const rawY = Math.round((ch - h) / 2);
    const { x: cx, y: cy } = clampTextZonePositionToCanvas(rawX, rawY, w, h, rot, 1080, ch);
    ...
    setExpandedTextSection(null);
    setExtraLayoutPopoverOpenZoneId(null);
  };

  const handleRemoveExtraTextZone = useCallback((zoneId: string, fromTemplateZone: boolean) => {
    if (fromTemplateZone) {
      setSuppressedExtraTextZoneIds((prev) => (prev.includes(zoneId) ? prev : [...prev, zoneId]));
      setExpandedExtraTextZoneId((prev) => (prev === zoneId ? null : prev));
      setExtraLayoutPopoverOpenZoneId((prev) => (prev === zoneId ? null : prev));
      return;
    }
    setCustomExtraTextZones((prev) => prev.filter((z) => z.id !== zoneId));
    setExtraTextValues((prev) => {
      const next = { ...prev };
      delete next[zoneId];
      return next;
    });
    setExtraTextHighlightsByZone((prev) => {
      const next = { ...prev };
      delete next[zoneId];
      return next;
    });
    setExtraTextHighlightStylesByZone((prev) => {
      const next = { ...prev };
      delete next[zoneId];
      return next;
    });
    setExtraTextOutlineStrokesByZone((prev) => {
      const next = { ...prev };
      delete next[zoneId];
      return next;
    });
    setExtraTextBoldWeightsByZone((prev) => {
      const next = { ...prev };
      delete next[zoneId];
      return next;
    });
    setSuppressedExtraTextZoneIds((prev) => prev.filter((id) => id !== zoneId));
    setExpandedExtraTextZoneId((prev) => (prev === zoneId ? null : prev));
    setExtraLayoutPopoverOpenZoneId((prev) => (prev === zoneId ? null : prev));
  }, []);
  const upsertExtraTextZonePatch = useCallback((zoneId: string, patch: Omit<Partial<ExtraTextZone>, "id">) => {
    const t = templateExtraTextZones.find((z) => z.id === zoneId);
    const c = customExtraTextZones.find((z) => z.id === zoneId);
    const fallbackBase =
      t != null ? ({ ...t, ...c } as ExtraTextZone) : c ?? extraTextZones.find((z) => z.id === zoneId);
  ...
  }, [templateExtraTextZones, customExtraTextZones, extraTextZones]);
  ...
  const applyHighlightToExtraSelection = useCallback(
    (zoneId: string, color: string, useSavedSelection?: boolean) => {
      const hex = color.startsWith("#") ? color : (HIGHLIGHT_COLORS[color] ?? "#facc15");
      const el = extraTextareaRefs.current[zoneId];
      let start: number;
      let end: number;
      if (useSavedSelection && savedHighlightSelectionRef.current?.field === zoneId) {
        const saved = savedHighlightSelectionRef.current;
        start = saved.start;
        end = saved.end;
        savedHighlightSelectionRef.current = null;
      ...
      if (start === end) return;
      const text = extraTextValues[zoneId] ?? "";
      const expanded = expandSelectionToWordBoundaries(text, start, end);
      if (!expanded) return;
      start = expanded.start;
      end = expanded.end;
      setExtraTextHighlightsByZone((prev) => {
        ...
        const next = cur.filter((s) => s.end <= start || s.start >= end);
        next.push({ start, end, color: hex });
        next.sort((a, b) => a.start - b.start);
  ...
  const applyBoldToExtraSelection = useCallback(
    (zoneId: string, useSavedSelection?: boolean) => {
      const el = extraTextareaRefs.current[zoneId];
      let start: number;
      let end: number;
      if (useSavedSelection && savedHighlightSelectionRef.current?.field === zoneId) {
      ...
      setExtraTextValues((prev) => ({ ...prev, [zoneId]: newText }));
      setExtraTextHighlightsByZone((prev) => {
        const cur = prev[zoneId];
        if (!cur?.length) return prev;
        return { ...prev, [zoneId]: shiftHighlightSpansForBoldInsert(cur, start, end) };
      });
      const newEnd = end + 4;
      setTimeout(() => {
        el?.focus();
  ...
  const saveHighlightSelectionForPicker = useCallback((target: "headline" | "body" | string) => {
    if (target === "headline") {
      const ref =
        headlineHighlightOpen && headlineModalRef.current ? headlineModalRef : headlineRef;
      const el = ref.current;
      if (el && typeof el.selectionStart === "number" && typeof el.selectionEnd === "number") {
        const start = el.selectionStart;
        const end = el.selectionEnd;
        if (start !== end) savedHighlightSelectionRef.current = { field: "headline", start, end };
      }
      return;
    }
    if (target === "body") {
      const ref = bodyHighlightOpen && bodyModalRef.current ? bodyModalRef : bodyRef;
      const el = ref.current;
      if (el && typeof el.selectionStart === "number" && typeof el.selectionEnd === "number") {
        const start = el.selectionStart;
        const end = el.selectionEnd;
        if (start !== end) savedHighlightSelectionRef.current = { field: "body", start, end };
      }
      return;
    }
    const el = extraTextareaRefs.current[target];
    if (el && typeof el.selectionStart === "number" && typeof el.selectionEnd === "number") {
      const start = el.selectionStart;
      const end = el.selectionEnd;
  ...
  const performSave = async (navigateBack = false) => {
      ...
      ...(hasImageForSave ? { allow_background_image_override: templateDisallowsImage } : {}),
          ...
          ...(madeWithZoneOverride && (() => {
            ...
            const filtered = {
              ...
              ...extractChromeChipStyle(o as Record<string, unknown>),
            };
          ...
          ...(Object.keys(extraTextValues).length > 0
            ...
            : {}),
          ...(customExtraTextZones.length > 0 ? { extra_text_zones: customExtraTextZones } : {}),
      overlay_shapes: slideOverlayShapes,
      ...(bodyZoneForRewrite && bodyRewriteVariants.length === 3 && {
        body_rewrite_variants: [...bodyRewriteVariants],
      }),
    };
    const extraZoneIdSetForSave = new Set<string>();
    for (const z of extraTextZones) extraZoneIdSetForSave.add(z.id);
    for (const z of customExtraTextZones) extraZoneIdSetForSave.add(z.id);
    for (const id of suppressedExtraTextZoneIds) extraZoneIdSetForSave.add(id);
    const extraZoneIds = [...extraZoneIdSetForSave];
    const extraHlSave: Record<string, { start: number; end: number; color: string }[]> = {};
    for (const id of extraZoneIds) {
      const spans = extraTextHighlightsByZone[id];
      if (!spans?.length) continue;
      const norm = normalizeHighlightSpansToWords(extraTextValues[id] ?? "", spans);
      if (norm.length > 0) extraHlSave[id] = norm;
    }
    if (Object.keys(extraHlSave).length > 0) metaForSave.extra_text_highlights = extraHlSave;
    else delete metaForSave.extra_text_highlights;

    const extraStylesSave: Record<string, "text" | "background"> = {};
    for (const id of extraZoneIds) {
      if (extraTextHighlightStylesByZone[id] === "background") extraStylesSave[id] = "background";
    }
    if (Object.keys(extraStylesSave).length > 0) metaForSave.extra_text_highlight_styles = extraStylesSave;
    else delete metaForSave.extra_text_highlight_styles;

    const extraOutlineSave: Record<string, number> = {};
    for (const id of extraZoneIds) {
      const v = extraTextOutlineStrokesByZone[id];
      if (typeof v === "number" && v > 0 && v <= 8) extraOutlineSave[id] = v;
    }
    if (Object.keys(extraOutlineSave).length > 0) metaForSave.extra_text_outline_strokes = extraOutlineSave;
    else delete metaForSave.extra_text_outline_strokes;

    const extraBoldSave: Record<string, number> = {};
    for (const id of extraZoneIds) {
      const w = extraTextBoldWeightsByZone[id];
      if (typeof w === "number" && w !== 700 && w >= FONT_WEIGHT_MIN && w <= FONT_WEIGHT_MAX) extraBoldSave[id] = Math.round(w);
    }
    if (Object.keys(extraBoldSave).length > 0) metaForSave.extra_text_bold_weights = extraBoldSave;
    else delete metaForSave.extra_text_bold_weights;
    ...
    const suppressedSave = [
      ...new Set(suppressedExtraTextZoneIds.filter((id) => typeof id === "string" && id.trim() !== "")),
    ];
    if (suppressedSave.length > 0) metaForSave.extra_text_zones_suppressed_ids = suppressedSave;
    else delete metaForSave.extra_text_zones_suppressed_ids;
  ...
  const handleLogoUpload = async (file: File | null) => {
    if (!file) return;
    const derivedProjectId = backHref.split("/")[2];
    if (!derivedProjectId) {
      setLogoUploadError("Could not determine project for logo upload.");
      return;
    }
    ...
    try {
      ...
      fd.set("logo", file);
      const result = await uploadProjectLogo(derivedProjectId, fd);
      if (!result.ok) {
        setLogoUploadError(result.error || "Could not upload logo.");
        return;
      }
      router.refresh();
    } catch {
  ...
  const handleRegenerateAiBackground = async () => {
    ...
    try {
      ...
      if (pool.length > 0) {
        ...
        setImageUrls((rows) => {
          if (rows.length === 0) return [{ url: r.backgroundImageUrl, storage_path: r.primaryStoragePath, alternates: [] }];
          const first = rows[0]!;
          return [{ ...first, url: r.backgroundImageUrl, storage_path: r.primaryStoragePath }, ...rows.slice(1)];
        });
      }
      router.refresh();
    } finally {
  ...
  const handleApplyWatermarkToAll = async () => {
    ...
    if (
      madeWithZoneOverride &&
      (madeWithZoneOverride.fontSize != null ||
        madeWithZoneOverride.x != null ||
        madeWithZoneOverride.y != null ||
        (madeWithZoneOverride.color != null && /^#([0-9A-Fa-f]{3}){1,2}$/.test(madeWithZoneOverride.color)) ||
        Object.keys(extractChromeChipStyle(madeWithZoneOverride as Record<string, unknown>)).length > 0)
    ) {
      const o = madeWithZoneOverride;
      const filtered = {
        ...(o.fontSize != null && { fontSize: o.fontSize }),
  ...
  const handleApplyExtraTextToAll = async (zoneId: string) => {
    const zone = allExtraTextZoneRows.find((r) => r.zone.id === zoneId)?.zone;
    if (!zone) return;
          ...
          {templateConfig ? (
            ...
            >
              <SlidePreview
                ...
                onTextZoneFocus={(zoneId) => {
                  setActiveEditZone(zoneId);
                  setEditorTab("text");
                  setExpandedExtraTextZoneId(zoneId);
                }}
                  ...
                  templateConfig
                    ? {
                        ...
                        fontWeight: clampFontWeight(
                          Number(headlineZoneOverride?.fontWeight ?? headlineZoneFromTemplate?.fontWeight ?? 600)
                        ),
                        ...
                        onFontWeightChange: (v) =>
                          setHeadlineZoneOverride((prev) => ({
                            ...headlineZoneFromTemplate,
                            ...prev,
                            fontWeight: clampFontWeight(typeof v === "number" ? v : Number(v)),
                          })),
                ...
                editToolbarBody={
                  templateConfig
                    ? {
                        ...
                        fontWeight: clampFontWeight(
                          Number(bodyZoneOverride?.fontWeight ?? bodyZoneFromTemplate?.fontWeight ?? 500)
                        ),
                  ...
                  {chromeLayoutOpen && (
                    <div className="space-y-4 mt-2">
                        ...
                        <div className="mt-3 border-t border-border/40 pt-3">
                          <p className="text-[10px] font-semibold uppercase tracking-wider text-muted-foreground mb-2">Font & backdrop</p>
                          <ChromeChipStyleEditor
                            value={extractChromeChipStyle(counterZoneOverride as Record<string, unknown> | undefined)}
                            onChange={(chip) =>
                              setCounterZoneOverride((o) =>
                                mergeChromeChipIntoZone((o ?? {}) as Record<string, unknown>, chip) as CounterZoneOverride | undefined
                              )
                            }
                            disabled={!showCounter}
                            fontModalTitle="Slide number font"
                          />
                        </div>
                        {totalSlides > 1 && isPro && (
                        ...
                        >
                          ...
                          {!brandKit.logo_url && (
                            <p className="text-[11px] text-muted-foreground mb-2 leading-snug">
                              No logo uploaded yet. Upload one here (or keep text-only handle below).
                            </p>
                          )}
                          <div className="mb-2">
                            <input
                            ...
                            >
                              {logoUploading ? <Loader2Icon className="size-3.5 animate-spin" /> : <UploadIcon className="size-3.5" />}
                              {brandKit.logo_url ? "Replace logo" : "Upload logo"}
                            </Button>
                            {logoUploadError && (
                              <p className="mt-1 text-[11px] text-destructive">{logoUploadError}</p>
                            )}
                          </div>
                          <div className="mb-2">
                            <Label className="text-xs">Color</Label>
                          ...
                          <div className="space-y-2">
                            <div className="grid grid-cols-2 gap-2">
                              ...
                              <div className="min-w-0">
                                ...
                                <StepperWithLongPress value={watermarkZoneOverride?.logoY ?? templateConfig?.chrome?.watermark?.logoY ?? 24} min={0} max={1920} step={8} onChange={(v) => setWatermarkZoneOverride((o) => ({ ...(o ?? {}), logoY: v, position: "custom" }))} label="Y" className="w-full min-w-0" disabled={!showWatermark} />
                              </div>
                            </div>
                            <div>
                              <Label className="text-xs">Font size</Label>
                              <StepperWithLongPress value={watermarkZoneOverride?.fontSize ?? 20} min={8} max={280} step={1} onChange={(v) => setWatermarkZoneOverride((o) => ({ ...o, fontSize: v }))} label="Font size" className="w-full min-w-0" disabled={!showWatermark} />
                            </div>
                          </div>
                          <div className="mt-3 border-t border-border/40 pt-3">
                            <p className="text-[10px] font-semibold uppercase tracking-wider text-muted-foreground mb-2">Font & backdrop</p>
                            <ChromeChipStyleEditor
                              value={extractChromeChipStyle(watermarkZoneOverride as Record<string, unknown> | undefined)}
                              onChange={(chip) =>
                                setWatermarkZoneOverride((o) =>
                                  mergeChromeChipIntoZone((o ?? {}) as Record<string, unknown>, chip) as WatermarkZoneOverride | undefined
                                )
                              }
                              disabled={!showWatermark}
                              fontModalTitle="Logo / watermark text font"
                            />
                          </div>
                          {totalSlides > 1 && isPro && (
                      ...
                      >
                        ...
                        <div className="space-y-3">
                          ...
                          <div className="grid grid-cols-2 gap-3">
                            <div>
                              <Label className="text-xs">Font size</Label>
                              <StepperWithLongPress value={madeWithZoneOverride?.fontSize ?? 30} min={12} max={280} step={1} onChange={(v) => setMadeWithZoneOverride((o) => ({ ...o, fontSize: v }))} label="Font size" className="w-full min-w-0" disabled={!showMadeWith} />
                            </div>
                            <div>
                              <Label className="text-xs">X (px)</Label>
                            ...
                            <div>
                              <Label className="text-xs">Y (px)</Label>
                              <StepperWithLongPress value={madeWithZoneOverride?.y ?? 1016} min={0} max={1900} step={4} onChange={(v) => setMadeWithZoneOverride((o) => ({ ...(o ?? {}), y: v }))} label="Y" className="w-full min-w-0" disabled={!showMadeWith} />
                            </div>
                          </div>
                          ...
                          <div className={cn("mt-3 border-t border-border/40 pt-3", !showMadeWith && "opacity-50 pointer-events-none")}>
                            <p className="text-[10px] font-semibold uppercase tracking-wider text-muted-foreground mb-2">Font & backdrop</p>
                            <ChromeChipStyleEditor
                              value={extractChromeChipStyle(madeWithZoneOverride as Record<string, unknown> | undefined)}
                              onChange={(chip) =>
                                setMadeWithZoneOverride((o) =>
                                  mergeChromeChipIntoZone((o ?? {}) as Record<string, unknown>, chip) as MadeWithZoneOverride | undefined
                                )
                              }
                              disabled={!showMadeWith}
                              fontModalTitle="Bottom handle font"
                            />
                          </div>
                          {totalSlides > 1 && isPro && (
            ...
            <div className="rounded-lg border border-border/50 bg-muted/5 p-3 space-y-2">
              <div className="flex items-center justify-between gap-2">
                ...
                <Button
                  ...
                  disabled={allExtraTextZoneRows.length >= 12}
                >
                  <PlusIcon className="size-3.5" />
                  Add text area ({allExtraTextZoneRows.length}/12)
                </Button>
              </div>
              <p className="text-[11px] text-muted-foreground">
              ...
              {allExtraTextZoneRows.length > 0 && (
                <div className="space-y-2">
                  {allExtraTextZoneRows.map(({ zone, fromTemplate }) => {
                    const hiddenOnSlide = suppressedExtraTextZoneSet.has(zone.id);
                    ...
                    const extraLayoutPopoverOpen = extraLayoutPopoverOpenZoneId === zone.id;
                    return (
                      <div
                        key={zone.id}
                        className={`rounded-md border bg-background/60 transition-colors ${activeEditZone === zone.id ? "border-primary/60 ring-1 ring-primary/30" : "border-border/60"} ${hiddenOnSlide ? "opacity-70" : ""}`}
                      >
                        <button
                          type="button"
                          ...
                          onClick={() => {
                            ...
                            setExpandedExtraTextZoneId((prev) => {
                        ...
                        <div className="flex flex-wrap items-center justify-between gap-2">
                          <div className="flex flex-wrap items-center gap-2">
                            ...
                            <label className="flex cursor-pointer items-center gap-1.5 text-[10px] text-muted-foreground">
                              <input
                                type="checkbox"
                                checked={!hiddenOnSlide}
                                onChange={(e) => {
                                  const show = e.target.checked;
                                  setSuppressedExtraTextZoneIds((prev) =>
                                    show ? prev.filter((id) => id !== zone.id) : prev.includes(zone.id) ? prev : [...prev, zone.id]
                                  );
                                }}
                                className="rounded border-input accent-primary"
                              />
                          ...
                          <div className="flex items-center gap-2">
                            ...
                            <Button
                              ...
                              onClick={() => handleRemoveExtraTextZone(zone.id, fromTemplate)}
                              title={fromTemplate ? "Hides this area from the slide; text and layout are kept" : "Remove this custom text area"}
                            >
                              {fromTemplate ? "Hide" : "Remove"}
                            </Button>
                          </div>
                        </div>
                        ...
                        <Textarea
                          ref={(el) => {
                            extraTextareaRefs.current[zone.id] = el;
                          }}
                        ...
                        <div className="space-y-2">
                          ...
                          <div className="flex flex-wrap items-center gap-2">
                            <Popover
                              open={extraLayoutPopoverOpen}
                              onOpenChange={(open) => {
                                if (open) {
                                  setHeadlineLayoutPopoverOpen(false);
                                  setBodyLayoutPopoverOpen(false);
                                }
                                setExtraLayoutPopoverOpenZoneId(open ? zone.id : null);
                              }}
                            >
                              ...
                              >
                                <div className="flex items-center justify-between gap-2 px-3 py-2 border-b border-border/50 shrink-0 bg-muted/30">
                                  <span className="text-xs font-medium text-foreground">{zone.label?.trim() || zone.id} position & layout</span>
                                  <div className="flex items-center gap-1">
                                    {fromTemplate && totalSlides > 1 && (
                                      <Button type="button" variant="outline" size="sm" className="h-7 text-xs gap-1" onClick={() => handleApplyExtraTextToAll(zone.id)} disabled={applyingExtraZoneId === zone.id} title="Apply this text value to all frames in scope">
                                        {applyingExtraZoneId === zone.id ? <Loader2Icon className="size-3.5 animate-spin" /> : <CopyIcon className="size-3.5" />}
                                        Apply to all
                                      </Button>
                                    )}
                                    <Button type="button" variant="ghost" size="sm" className="h-7 w-7 p-0 text-xs" onClick={() => setExtraLayoutPopoverOpenZoneId(null)} aria-label="Close">
                                      ×
                                    </Button>
                                  </div>
                                </div>
                                            ...
                                            <StepperWithLongPress
                                              ...
                                              onChange={(v) => {
                                                if (key === "h") {
                                                  ...
                                                  updateZone({ [key]: v } as Partial<ExtraTextZone>);
                                                }
                                              }}
                                  ...
                                  <div>
                                    ...
                                    <div className="grid grid-cols-2 gap-2 sm:gap-4 sm:grid-cols-4">
                                      ...
                                      <div className="space-y-1 sm:space-y-1.5 min-w-0">
                                        ...
                                        <StepperWithLongPress
                                          value={clampFontWeight(Number(zone.fontWeight))}
                                          min={FONT_WEIGHT_MIN}
                                          max={FONT_WEIGHT_MAX}
                                          step={100}
                                          onChange={(v) => updateZone({ fontWeight: clampFontWeight(v) })}
                                          label="font weight"
                                          className="w-full min-w-0"
                                        />
                                      </div>
                                      <div className="space-y-1 sm:space-y-1.5 min-w-0">
                                        ...
                                        <StepperWithLongPress
                                          value={Math.round((zone.lineHeight ?? 1.15) * 20) / 20}
                                          min={0.5}
                                          max={3}
                                          step={0.05}
                                          onChange={(v) => {
                                      ...
                                      <div className="space-y-1 sm:space-y-1.5 min-w-0">
                                        ...
                                        <StepperWithLongPress
                                          ...
                                          max={180}
                                          step={5}
                                          onChange={(v) => updateZone({ rotation: v })}
                        ...
                        <div className="space-y-2">
                          ...
                          <div className="flex flex-wrap items-center gap-1.5">
                            <Button
                              ...
                              onClick={() => applyBoldToExtraSelection(zone.id, true)}
                              onMouseDown={() => saveHighlightSelectionForPicker(zone.id)}
                              title="Bold selected word(s)"
                            >
                            ...
                            {(["yellow", "amber", "orange", "lime", "cyan", "white"] as const).map((preset) => (
                              <button
                                ...
                                onMouseDown={(e) => {
                                  e.preventDefault();
                                  saveHighlightSelectionForPicker(zone.id);
                                }}
                                onClick={() => applyHighlightToExtraSelection(zone.id, HIGHLIGHT_COLORS[preset] ?? "#facc15", true)}
                                className="h-6 w-6 rounded-full border-2 shrink-0 hover:scale-110 transition-transform border-transparent"
                            ...
                            <label
                              className="h-6 w-6 rounded-full border-2 border-transparent shrink-0 cursor-pointer hover:scale-110 transition-transform inline-block overflow-hidden [&:has(input:focus)]:ring-2 [&:has(input:focus)]:ring-primary/50"
                              style={{ background: HIGHLIGHT_RAINBOW }}
                              title="Custom highlight color"
                            >
                              <input
                                type="color"
                                className="w-full h-full opacity-0 cursor-pointer block"
                                value={defaultHighlightColor.startsWith("#") ? defaultHighlightColor : "#facc15"}
                                onChange={(e) => {
                                  const v = e.target.value;
                                  if (savedHighlightSelectionRef.current?.field === zone.id) applyHighlightToExtraSelection(zone.id, v, true);
                                }}
                                onMouseDown={() => saveHighlightSelectionForPicker(zone.id)}
                            ...
                            <div className="flex rounded border border-border/60 overflow-hidden shrink-0">
                              {(["text", "background"] as const).map((style) => (
                                <Button
                                  key={style}
                                  type="button"
                                  variant={(extraTextHighlightStylesByZone[zone.id] ?? "text") === style ? "secondary" : "ghost"}
                                  size="sm"
                                  className="h-6 text-[11px] px-2 rounded-none first:rounded-l last:rounded-r"
                                  onClick={() =>
                                    setExtraTextHighlightStylesByZone((prev) => ({
                                      ...prev,
                                      [zone.id]: style,
                                  ...
                                  title={style === "text" ? "Colored text" : "Colored background"}
                                >
                                  {style === "text" ? "Text" : "Bg"}
                                </Button>
                              ))}
                        ...
                        <div className="space-y-2">
                          ...
                          <div className="flex items-center gap-2">
                            ...
                            <Button
                              type="button"
                              variant={(extraTextOutlineStrokesByZone[zone.id] ?? 0) > 0 ? "secondary" : "ghost"}
                              size="sm"
                              className="h-7 text-[11px] px-2 shrink-0"
                              onClick={() =>
                                setExtraTextOutlineStrokesByZone((prev) => ({
                                  ...prev,
                                  [zone.id]: (prev[zone.id] ?? 0) > 0 ? 0 : 2,
                                }))
                              }
                              title={(extraTextOutlineStrokesByZone[zone.id] ?? 0) > 0 ? "Turn outline off" : "Turn outline on"}
                            >
                              {(extraTextOutlineStrokesByZone[zone.id] ?? 0) > 0 ? "On" : "Off"}
                            </Button>
                            <div className="flex items-center rounded border border-border/60 overflow-hidden">
                              <Button
                                ...
                                onClick={() =>
                                  setExtraTextOutlineStrokesByZone((prev) => ({
                                    ...prev,
                                    [zone.id]: Math.max(0, (prev[zone.id] ?? 0) - 0.5),
                                  }))
                                }
                                aria-label="Decrease outline size"
                                disabled={(extraTextOutlineStrokesByZone[zone.id] ?? 0) === 0}
                              >
                                −
                              </Button>
                              <span className="min-w-[2.5rem] text-center text-xs tabular-nums px-1">{extraTextOutlineStrokesByZone[zone.id] ?? 0}</span>
                              <Button
                                type="button"
                                variant="ghost"
                                size="sm"
                                ...
                                onClick={() =>
                                  setExtraTextOutlineStrokesByZone((prev) => ({
                                    ...prev,
                                    [zone.id]: Math.min(8, (prev[zone.id] ?? 0) + 0.5),
                                  }))
                                }
                                aria-label="Increase outline size"
                              >
                                +
                              </Button>
                            </div>
                          </div>
                          {(extraTextValues[zone.id] ?? "").includes("**") && (
                              ...
                              <div className="flex items-center rounded border border-border/60 overflow-hidden">
                                <Button
                                  ...
                                  onClick={() =>
                                    setExtraTextBoldWeightsByZone((prev) => ({
                                      ...prev,
                                      [zone.id]: Math.max(FONT_WEIGHT_MIN, Math.min(FONT_WEIGHT_MAX, (prev[zone.id] ?? 700) - 100)),
                                    }))
                                  }
                                  aria-label="Decrease bold weight"
                                >
                                  −
                                </Button>
                                <span className="min-w-[2.5rem] text-center text-xs tabular-nums px-1">{extraTextBoldWeightsByZone[zone.id] ?? 700}</span>
                                <Button
                                  type="button"
                                  variant="ghost"
                                  size="sm"
                                  ...
                                  onClick={() =>
                                    setExtraTextBoldWeightsByZone((prev) => ({
                                      ...prev,
                                      [zone.id]: Math.min(FONT_WEIGHT_MAX, Math.max(FONT_WEIGHT_MIN, (prev[zone.id] ?? 700) + 100)),
                                    }))
                                  }
                                  aria-label="Increase bold weight"
                                >
                                  +
                                </Button>
                              </div>
                          ...
                          <div className="flex flex-col gap-2 sm:flex-row sm:items-start sm:justify-between sm:gap-3">
                            ...
                            >
                              ...
                              <button
                                ...
                                onClick={() => {
                                  const cur = zone.boxBackgroundColor?.trim();
                                  const hasValid = !!cur && TEXT_BACKDROP_HEX_RE.test(cur);
                                  const prevOp = zone.boxBackgroundOpacity;
                                  const keepOpacity = typeof prevOp === "number" && !Number.isNaN(prevOp) && hasValid;
                                  updateZone({
                            ...
                            <div className="flex flex-col gap-2.5 sm:flex-row sm:items-center sm:gap-3">
                                  ...
                                  <ColorPicker
                                    ...
                                    onChange={(v) => {
                                      ...
                                      updateZone({
                                        ...
                                        boxBackgroundOpacity: zone.boxBackgroundOpacity ?? DEFAULT_TEXT_BACKDROP_OPACITY,
                                      });
                                    }}
                                    placeholder="#000000"
                                    compact
                              ...
                              <div className="flex flex-1 items-center gap-2 min-w-0 min-h-9">
                                ...
                                <Slider
                                  ...
                                  max={100}
                                  step={1}
                                  value={[Math.round((zone.boxBackgroundOpacity ?? DEFAULT_TEXT_BACKDROP_OPACITY) * 100)]}
                                  onValueChange={(vals) => {
                                ...
                                <span className="text-[11px] tabular-nums text-muted-foreground w-10 text-right shrink-0">
                                  {Math.round((zone.boxBackgroundOpacity ?? DEFAULT_TEXT_BACKDROP_OPACITY) * 100)}%
                                </span>
                              </div>
                            </div>
                          )}
                          {textBackdropIsOn(zone) && (
                            <TextBackdropChromeFields zone={zone} onMerge={(patch) => updateZone(patch as Partial<ExtraTextZone>)} />
                          )}
                        </div>
                        </div>
                        </div>
                      ...
                      <>
                                        ...
                                        <div className="space-y-1 sm:space-y-1.5 min-w-0">
                                          ...
                                          <StepperWithLongPress
                                            value={clampFontWeight(
                                              Number(
                                                headlineZoneOverride?.fontWeight ??
                                                  (effectiveHeadlineZoneBase ?? templateConfig!.textZones!.find((z) => z.id === "headline")!)
                                                    .fontWeight
                                              )
                                            )}
                                            min={FONT_WEIGHT_MIN}
                        ...
                        {(headline ?? "").includes("**") && (
                          <div className="flex items-center gap-2 pt-0.5">
                            <span className="text-[11px] text-muted-foreground shrink-0">Bold weight</span>
                            <div className="flex items-center rounded border border-border/60 overflow-hidden">
                              <Button type="button" variant="ghost" size="sm" className="h-7 w-7 rounded-none shrink-0" onClick={() => setHeadlineBoldWeight((v) => clampFontWeight(v - 100))} aria-label="Decrease bold weight">−</Button>
                              <span className="min-w-[2.5rem] text-center text-xs tabular-nums px-1">{clampFontWeight(headlineBoldWeight)}</span>
                              <Button type="button" variant="ghost" size="sm" className="h-7 w-7 rounded-none shrink-0" onClick={() => setHeadlineBoldWeight((v) => clampFontWeight(v + 100))} aria-label="Increase bold weight">+</Button>
                            </div>
                          </div>
                        )}
                        </div>
                      ...
                      <>
                                      ...
                                      <div className="grid grid-cols-2 gap-2 sm:gap-4 sm:grid-cols-4">
                                        ...
                                        <div className="space-y-1 sm:space-y-1.5 min-w-0">
                                          ...
                                          <StepperWithLongPress
                                            value={clampFontWeight(
                                              Number(
                                                bodyZoneOverride?.fontWeight ??
                                                  (effectiveBodyZoneBase ?? templateConfig!.textZones!.find((z) => z.id === "body")!).fontWeight
                                              )
                                            )}
                                            min={FONT_WEIGHT_MIN}
                                            max={FONT_WEIGHT_MAX}
                                            ...
                                            onChange={(v) =>
                                              setBodyZoneOverride((o) => ({
                                                ...(effectiveBodyZoneBase ?? templateConfig!.textZones!.find((z) => z.id === "body")!),
                                                ...o,
                                                fontWeight: clampFontWeight(v),
                                              }))
                                            }
                        ...
                        {(body ?? "").includes("**") && (
                          <div className="flex items-center gap-2 pt-0.5">
                            <span className="text-[11px] text-muted-foreground shrink-0">Bold weight</span>
                            <div className="flex items-center rounded border border-border/60 overflow-hidden">
                              <Button type="button" variant="ghost" size="sm" className="h-7 w-7 rounded-none shrink-0" onClick={() => setBodyBoldWeight((v) => clampFontWeight(v - 100))} aria-label="Decrease bold weight">−</Button>
                              <span className="min-w-[2.5rem] text-center text-xs tabular-nums px-1">{clampFontWeight(bodyBoldWeight)}</span>
                              <Button type="button" variant="ghost" size="sm" className="h-7 w-7 rounded-none shrink-0" onClick={() => setBodyBoldWeight((v) => clampFontWeight(v + 100))} aria-label="Increase bold weight">+</Button>
                            </div>
                          </div>
                        )}
                        </div>
