...
function deterministicFallback(params: {
  ...
  const inventedCastLock =
    " **Recurring invented people:** when the same protagonist, narrator, guide, couple, or presenter appears across slides (storytelling, tutorial host, product demo face, etc.—not a list of unrelated real public figures), keep **one stable identity**: same face read, hair, skin tone, age, build, **distinctive wardrobe palette** (interesting color/layer read, not washed-out generic basics), jewelry, and recurring companions until the copy changes character or time; vary pose, expression, framing, and setting—not a new actor each slide.";
