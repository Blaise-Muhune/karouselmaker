...
function deterministicFallback(params: {
  ...
  if (params.ugcMode && !params.preferRecognizablePublicFigures) {
    ...
    return `UGC / smartphone look across ${params.slideCount} slides for ${anchor}: default to **natural iPhone realism**—visible light sensor grain in medium indoor light, soft focus (not razor-sharp), muted colors, flat practical lighting (e.g. gym overheads, desk lamp), realistic skin texture—**not** studio HDR, beauty retouch, catalog gloss, “perfect AI face,” or suspiciously convenient staging (plastic skin, empty designer sets, buttery HDR).${seedBit} Camera: natural phone POV (eye level, arm’s length, slight high/low); mirror selfie or bathroom/gym mirror only when the scene fits—no crane, drone, or staged hero angles unless the topic demands it. Each slide follows the **story beat** (setting and props from the slide text).${envLock} **Character lock** when a person appears: one recurring creator—identical face shape, features, hair (color/length/style), skin tone, body type, age read, and recurring casual outfit palette; vary only pose, expression, framing, and room. No text in images.`;
  }
