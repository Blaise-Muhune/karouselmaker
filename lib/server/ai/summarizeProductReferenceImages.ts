...
- For UI/screenshots: layout regions, key components (nav, hero, cards, charts), approximate text density (do not transcribe long text), primary accent colors, light vs dark mode, device frame if visible. In one short clause, name the **job-to-be-done** the UI suggests (e.g. invoicing, scheduling, code review, habit tracking)—so downstream copy stays outcome-led, not meaningless “dashboard” filler.
