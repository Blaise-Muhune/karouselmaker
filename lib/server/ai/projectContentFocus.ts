...
**Software / app / SaaS (when you’re promoting a digital tool, not only a physical product):** Most slides should still **deliver standalone value**—relatable work-life beats, habits, mistakes, mindset, or “what actually changed” before the tool gets a natural name-drop. The product should feel like the **honest bridge** to a calmer/faster/clearer outcome, **not** a jargon feature stack, fake metrics, or six slides of UI chrome trivia. Talk about it **greatly** where it earns the moment (proof, objection, “what I reach for when…”), in plain words a friend would use.
**Images (when AI backgrounds):** Natural iPhone-style shots—slight grain in medium light, soft focus, muted colors, practical indoor light when indoors—**not** studio HDR, beauty retouch, catalog gloss, plastic “AI” faces, or fake-perfect staging unless the slide copy clearly calls for it. If notes ask for studio/cinematic, follow that. image_queries: believable light and angles (eye level, arm’s length, mirror/gym only when the copy fits); match each slide’s headline and body. **One recurring creator** when a person appears—same face, hair, skin, build; **wardrobe** should feel **intentional and interesting** (color story, layers, footwear, or one standout accessory that fits the niche)—not anonymous grey-on-grey basics every slide; vary pose and scene, and vary outfit **when** the story timeline allows (keep one locked look for same-day / same-outing spans). For digital products: **interleave** creator + desk/couch/cafe beats with **occasional** believable phone/laptop UI moments when the copy calls for the tool—not identical full-screen mockups on every slide.
LAST SLIDE / CTA (UGC): Always slide_type "cta" with a real next step. If a product/service is in play, plain language (link in bio, DM, try it)—no URLs on slides; creator_handle when it fits. Otherwise follow/save/share in a human way—**one primary ask**, optional short second line; not a billboard stack. Sound like you’re wrapping the convo naturally.`;
    case "product_placement":
      return `CONTENT STYLE — product placement: **Sell the want first** (relief, confidence, time saved, looking put-together), then let the product be the believable bridge in the story—problem → lived moment → “what I actually use” → outcome. Avoid screaming “buy now” on every slide; one or two slides can be more direct, the rest should feel like context and proof. image_queries: product-in-environment, hands using it, outcome scenes, packaging on a real desk, UI/device close-ups—matched to each slide’s **emotional beat**, not catalog shots every time. Only make people/faces the main visual when the slide copy clearly calls for a presenter/customer moment. Reflect this placement mindset across several slides (fair coverage in headlines, body, and visuals) without repeating the same sales line.
LAST SLIDE / CTA (product placement): Always end with slide_type "cta". When the input or project rules name what you’re promoting, **headline = outcome or “when you’re ready…”**; **body** = soft next step (try, bio, DM, start)—plain language, **no URLs on slides**; use creator_handle when appropriate. When nothing concrete is named, close with a strong platform CTA (follow/save/share). Sound **confident and human**, not desperate or repetitive with earlier slides.`;
    case "educational":
...
export function contentFocusTopicHint(id: ContentFocusId): string {
  switch (id) {
    case "ugc":
      return "Bias topics toward authentic UGC angles—personal takes, relatable rants, ‘things I learned’, day-in-the-life energy, not corporate campaigns. When the project promotes software, bias toward **workflow and felt outcomes** (focus, deadlines, team mess, “before I had one place for…”) so the app names itself naturally—not “five features of our platform.”";
    case "product_placement":
...
export function contentFocusImagePipelineLine(id: ContentFocusId): string {
  switch (id) {
    case "ugc":
      return "Project content style: UGC—default natural phone photo (slight grain, soft focus, muted colors, practical light; not studio HDR, beauty retouch, or AI-smooth stock look). Avoid overly convenient or synthetic compositions. Natural angles: eye level / arm’s length / slight tilt; mirror selfie only if scene fits. Strict same-person lock when a human appears: same face shape, hair, skin tone, build, age read; **outfit palette** should be **distinctive and scroll-stopping** (clear colors, layers, accessories that fit the topic)—avoid bland default neutral basics unless notes say minimal; same-day spans still lock one outfit. Vary pose/scene. Override to studio/cinematic/commercial only if notes explicitly request it.";
    case "product_placement":
...
export function contentFocusHookHint(id: ContentFocusId): string {
  switch (id) {
    case "ugc":
      return "Content style: UGC—hook and body sound like one creator (natural, not corporate); don’t crank influencer voice to 11. If the carousel sells an app/tool: still lead with relatable value and moments—product praise should feel earned, not like release notes.";
    case "product_placement":
