...
type PromptContext = {
  ...
  /** When true and use_ai_backgrounds is true, images are generated via OpenAI—use descriptive generation prompts, not search keywords. */
...
${ctx.use_ai_backgrounds ? (ctx.use_ai_generate
  ? `- CRITICAL: EVERY slide MUST have image_queries (array with at least 1 string). These are PROMPTS for AI image generation—NOT search keywords. Write descriptive, visual prompts that tell the model what to create.
  • IMAGE GENERATION PROMPTS (not search): Each image_query is fed to an AI image model. Write short descriptive prompts: subject + setting + style. Do NOT use search terms like "4k", "3000x2000", "official photo"—those are for image search, not generation.
  • DRAMA / CINEMATIC STYLE (capture attention): Add lighting and mood cues so images feel intriguing and scroll-stopping. Use: "dramatic lighting", "golden hour", "cinematic", "atmospheric", "warm backlighting", "soft bokeh background", "intense close-up", "low angle", "mysterious mood", "celebratory", "high-stakes moment". Examples: "Kylian Mbappé in France jersey, dramatic golden backlighting, cinematic portrait", "Chef presenting chocolate dessert with liquid nitrogen smoke, fine dining, atmospheric lighting", "Mountain landscape at golden hour, cinematic and awe-inspiring", "Person in shock holding a note, gritty bathroom, dramatic expression, intense close-up".
  • DESCRIBE THE SCENE: Include subject (who/what), setting (where/context), and at least one drama/cinematic cue (lighting, mood, or composition). E.g. "Soccer trophy on dark background, dramatic spotlight" or "Woman blowing a kiss, golden halo backlighting, intimate and magical mood".
  • 1 IMAGE: almost always. One prompt per slide. 2 IMAGES: only when the slide compares two distinct things.
  • FIRST SLIDE (hook): Prompt must describe the main subject—person, theme, or setting—with dramatic or cinematic styling. Support the hook visually; make it attention-grabbing.
  • BIBLE / CHRISTIANITY / RELIGIOUS (first slide only): Do NOT use a generic "Bible", "open Bible", or "Bible on table" for the hook—it is not interesting and people scroll away. Use a specific Bible character, story event, or figure related to the user's topic instead (e.g. "David and Goliath moment", "Moses at the Red Sea", "Daniel in the lions' den", "prophet in the wilderness", "Jesus with the children", "Paul on the road to Damascus"—pick one that fits the carousel). The hook image must be a person or scene, never just the physical book.
  • LAST SLIDE (CTA): Topic-related prompt—same world as the carousel, with cinematic or atmospheric touch.
  • ALWAYS TOPIC-RELATED: Every image_query MUST describe something from the carousel topic or the slide—never use "nature landscape peaceful", "peaceful nature", or generic landscape when the topic is not about nature/travel. E.g. for Spider-Man villains use a villain or scene (e.g. "Venom symbiote dramatic scene", "Doctor Octopus lab"); for sports use the sport/player; for movies use the film or character. If the slide is a CTA, still use a topic-related visual (same world as the carousel), not a generic nature image.
  • Keep prompts under ~100 chars when possible. One clear visual idea per prompt. No text, logos, or watermarks in the description.
  • CONTEXT FOR ACCURACY (year/location): When the topic has a specific time period or place (e.g. "2024 Olympics", "Paris", "1980s", "World Cup 2022"), output image_context on the relevant slides so generated images match the right era and setting. Example: image_context: {"year":"2024","location":"Paris"} or {"year":"1980s"}. Omit image_context when time/place are generic or irrelevant.`
  : ctx.use_unsplash_only
...
{"title":"string","slides":[{"slide_index":1,"slide_type":"hook|point|context|cta|generic","headline":"string","body":"string or omit","headline_highlight_words":["word1","word2"],"body_highlight_words":["word1"],"shorten_alternates":[{"headline":"string","body":"string or omit","headline_highlight_words":["word"],"body_highlight_words":["word"]},...]${ctx.use_ai_backgrounds ? ',"image_queries":["phrase"]' : ""}}],"caption_variants":{"short":"string","medium":"string","spicy":"string"},"hashtags":["string"]}`;
  ...
  const viralShortsUserNote = ctx.viral_shorts_style
    ? " CRITICAL: Generate in Viral Shorts style: (1) Bait hook (e.g. \"This [person] did a huge mistake because...\"), (2) Story build-up slides, (3) ONE mid-carousel engagement slide (e.g. \"Which superpower do you want? Comment below.\" or \"What would you add?\"), (4) Payoff slides, (5) End CTA must be follow/subscribe (e.g. \"Follow @handle\", \"Subscribe for more\"). 8–14 slides. No format words in slide text."
    : "";

  const user = `${slideCountInstruction}
Use a curiosity-driven title: make people want to stick around until the end to get the answer (e.g. "What X said at the end is wild", "The last one changes everything"). Last slide: must always be a follow/subscribe CTA (e.g. "Follow @handle", "Subscribe for more") with optional short tagline (e.g. "More [niche] every week.")—short and fast, no long copy. No "swipe", "scroll", or "watch" in slide text—works for both carousels and voiceover.${viralShortsUserNote}
...
${ctx.use_ai_backgrounds ? (ctx.use_ai_generate
  ? "CRITICAL: Every slide MUST have image_queries. These are AI IMAGE GENERATION prompts—write descriptive, visual prompts with drama/cinematic cues (lighting, mood, composition). E.g. 'Kylian Mbappé in France jersey, dramatic golden backlighting, cinematic portrait'. Add 'dramatic', 'cinematic', 'golden hour', or 'atmospheric' to make images scroll-stopping. NOT search terms like '4k' or '3000x2000'."
  : ctx.use_unsplash_only
