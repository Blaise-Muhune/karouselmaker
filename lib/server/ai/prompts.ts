...
${ctx.use_ai_backgrounds ? (ctx.use_unsplash_only
  ...
  • EVERY SLIDE: The query subject must be the topic or a topic-relevant person/thing—e.g. for a slide about "film, trim, post" use "video editing setup 3000x2000" or "content creator filming press kit"; for "calm face" use a person with calm expression in that topic context. For how-to or step-by-step slides, match the image to the specific step (e.g. that exercise, that camera angle, that dish)—not a generic topic image. No stadiums, abstract gradients, or decorative art unless the topic is literally about that. For productivity/study/learning topics avoid overwhelming the viewer—prefer calming visuals (study space, peaceful workspace, focus, soft light) over literal screenshots or author photos on every slide.
...
${ctx.use_ai_backgrounds ? (ctx.use_unsplash_only
  ...
  : "CRITICAL: Every slide MUST have image_queries. Images must be about the CAROUSEL TOPIC and the slide—never generic. For any topic use specific, recognizable subjects (named people, well-known figures, iconic things) and quality cues (3000x2000, 4k, high quality, or \"official photos\"; for sports do NOT use \"press kit\" or \"kit\"—they return jersey/uniform) so results are high-quality and recognizable—especially for non-ranking topics like 'why X' or 'how Y', use actual players, coaches, celebrities, or experts, not generic labels. Prefer a person relevant to the topic. When the subject is a person (especially first name or common name), add disambiguating details: full name, team, country, role, or sport (e.g. rodri man city spain 3000x2000 photo, not just rodri 3000x2000). Never use only a single name for athletes (e.g. not \"Pele 3000x2000\")—use \"Pelé Brazil footballer\" or \"Pelé soccer legend\" so the correct person is returned. Use Brave patterns: subject + (1) 3000x2000 photo, (2) filetype:jpg 3000, (3) site or official photos, (4) \"official photos\" (for sports do NOT use \"press kit\" or \"kit\"—returns jersey/uniform), (5) \"full resolution\" or \"DSLR\" photo. For comic books, fictional characters, anime, superheroes: do NOT use press photo/official photo/press kit; do NOT use \"artwork\" or \"key art\" (return canvas/drawn or merchandise). Use \"wallpaper\" or \"poster\" only—do NOT use \"images\"; one term per query, never combine. For religious/historical figures and Bible topics (e.g. Jesus, Moses): use \"wallpaper\" or \"high resolution\"—do NOT add \"painting\" unless the topic is explicitly about paintings or art. Only include \"painting\" when the user is researching art/paintings specifically. Never use query terms that return product or merchandise (no \"canvas print\", \"t-shirt\", \"mug\", \"wall art for sale\", \"merchandise\", Etsy/Redbubble style)—we want the actual subject (photo/image), not designs on products. Prefer \"wallpapers\" for variety (avoid \"artwork\"—often returns canvas/drawn style; do not use \"images\"). Hook (slide 1): image must be the MAIN SUBJECT of the carousel (e.g. if about Jesus use Jesus, not the Bible; if about a person use that person). First slide = the face/figure that converts—no generic objects when the topic has a clear hero or character. For football/soccer: do NOT use Messi for slide 1 or last slide by default. Use Ronaldo, Mbappé, Salah, Haaland, Vinicius Jr, Bellingham, or a coach instead. Vary across carousels. After slide 1 go straight to the points—no wind-up. Last slide: same—for football do not default to Messi; use another star or coach. For movies/music: rotate among different actors/artists. Iconic, recognizable subject that fits the topic. Film/editing slides: filming or video editing imagery. For productivity/study/learning topics: prefer calming, appealing imagery (calm study space, peaceful workspace, focus, soft light)—do not use literal app screenshots or author headshots on every slide; keep it inviting and easy on the eye. For how-to or step-by-step: match each slide's image to that step (e.g. gym slide = someone doing that exercise; camera slide = that angle or technique; cooking = that dish or technique). News/current events: use the real subject of the story (person, event, place) for image_queries—e.g. [person] 3000x2000 photo, [event] press—not generic news imagery. No stadiums, abstract gradients, or random stock.") : ""}
