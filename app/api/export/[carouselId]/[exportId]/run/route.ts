...
import { renderSlideHtml, type SlideBackgroundOverride } from "@/lib/server/renderer/renderSlideHtml";
  ...
  try {
    ...
    try {
      for (let i = 0; i < slides.length; i++) {
        ...
        const gradientDirection = ["top", "bottom", "left", "right"].includes(rawDirection) ? rawDirection as "top" | "bottom" | "left" | "right" : "bottom";
        const overlayFields = {
          gradientStrength,
          gradientColor,
        ...
        const gradientOn: boolean = slideBg
          ? (slideBg.gradientOn === true || slideBg.gradientOn === false
              ? slideBg.gradientOn
              : (slideBg.overlay as { gradient?: boolean })?.gradient ?? true)
          : true;
        ...
        const showWatermarkOverride =
          slideMeta?.show_watermark === true || slideMeta?.show_watermark === false ? slideMeta.show_watermark : defaultShowWatermark;
        const showMadeWithOverride =
          slideMeta?.show_made_with === true || slideMeta?.show_made_with === false ? slideMeta.show_made_with : !isPro;
        ...
        const zoneOverrides: TextZoneOverrides | undefined =
          slideMeta && (slideMeta.headline_zone_override || slideMeta.body_zone_override)
            ? { headline: slideMeta.headline_zone_override as TextZoneOverrides["headline"], body: slideMeta.body_zone_override as TextZoneOverrides["body"] }
            : undefined;
