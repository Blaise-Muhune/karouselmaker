...
export async function GET(request: Request) {
  ...
  response.cookies.set("tiktok_oauth_return_to", safeReturnTo(new URL(request.url).searchParams.get("return_to")), { httpOnly: true, sameSite: "lax", secure: true, maxAge: 600, path: "/" });
  return response;
}
