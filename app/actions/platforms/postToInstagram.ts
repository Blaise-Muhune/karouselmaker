...
export async function postToInstagram(
  ...
  if (!readyExport.storage_path) {
    return { ok: false, error: "Export files are not stored. Use the downloaded ZIP and upload the images to Instagram manually, or use Post video to Instagram with a generated video." };
  }
  ...
  const captionToPost = caption?.trim() || getCaptionAndHashtagsForPost(carousel);

  try {
    const result = await postCarouselToInstagram(igAccountId, pageToken, imageUrls, captionToPost || undefined);
    return {
      ok: true,
